...
import '../../features/base/base_bloc_stateful_widget.dart';
...
class LatestConcertsEvents extends BaseBlocEvent {}

class LatestConcertsFetchData extends LatestConcertsEvents {
  EventRangeModel eventRangeModel;
  Language language;
  LatestConcertsFetchData(this.eventRangeModel, this.language);
}
...
class LatestConcertsVisible extends LatestConcertsEvents {}

class LatestConcertsStateSuccess extends BaseBlocState {
  final List<EventModel> result;
  final int currentItemIndex;
  LatestConcertsStateSuccess(
      {required this.result, required this.currentItemIndex});
}

@singleton
class LatestConcertsBloc extends BaseBloc<LatestConcertsEvents, BaseBlocState> {
  final EventRepository eventRepo;
  LatestConcertsBloc(this.eventRepo) {
    on<LatestConcertsFetchData>((event, emit) async {
      try {
        emit(StateLoading());
        ...
        if (result is Success<List<EventModel>>) {
          emit(LatestConcertsStateSuccess(
              result: result.data, currentItemIndex: 0));
    ...
    on<ChangeCurrentItem>((event, emit) {
      if (state is LatestConcertsStateSuccess) {
        emit(LatestConcertsStateSuccess(
          result: (state as LatestConcertsStateSuccess).result,
  ...
  bool eventToDebounce(BaseBlocEvent currentEvent) {
    return currentEvent is LatestConcertsFetchData;
  }

  @override
  Duration eventDebounceThreshold(
      BaseBlocEvent lastEvent, BaseBlocEvent event) {
    if (event is LatestConcertsFetchData) {
      if (lastEvent is LatestConcertsFetchData &&
          (
  ...
  int getEventListLength() {
    if (state is LatestConcertsStateSuccess) {
      return (state as LatestConcertsStateSuccess).result.length;
    } else {
      return -1;
    }
  }
  ...
  int getCurrentItemIndex() {
    if (state is LatestConcertsStateSuccess) {
      return (state as LatestConcertsStateSuccess).currentItemIndex;
    } else {
      return -1;
    }
  }
...
class LatestConcerts extends BaseBlocStatefulWidget {
  ...
  @override
  State<LatestConcerts> createState() => _LatestConcertsState();
}

class _LatestConcertsState
    extends BaseBlocStateWidget<LatestConcerts, LatestConcertsBloc> {
  final ScrollController _scrollController = ScrollController();
  late LatestConcertsBloc bloc;
  @override
  void preConfigBloc(BuildContext context, LatestConcertsBloc bloc) {
    this.bloc = bloc;
  }
  ...
  void _scrollNext(BuildContext context) {
    ...
    if (currentItemIndex < eventListLength - 1 &&
      ...
      addEvent(
          context, ChangeCurrentItem(currentItemIndex + widget.scrollCount));
    } else {
      _scrollToItem(currentItemIndex);
  ...
  @override
  Key get visibilityKey => Key("latest concerts");

  @override
  void onVisible(BuildContext context) {
    if (kDebugMode) {
      print("Latest Concerts is Visible");
    }
    addEvent(context, LatestConcertsVisible());
  }

  @override
  void onInvisible(BuildContext context) {
    if (kDebugMode) {
      print("Latest Concerts is InVisible");
    }
  }
  ...
  Widget body(BuildContext context, BaseBlocState state) {
    ...
    } else if (state is LatestConcertsStateSuccess) {
